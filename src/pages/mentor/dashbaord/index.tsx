...
import { deleteCourse, fetchCoursesByCreator, updateCourse } from '../../../redux/slices/mentorSlice';
...
function Index() {
    ...
    const { creatorCourses = [], loading, error } = useSelector((state: any) => state.mentor);

    const handleDeleteCourse = async (courseId: string) => {
        startTransition(() => {
            dispatch(deleteCourse({ courseId, headers: { Authorization: `Bearer ${authToken}` } }))
    ...
    const handleEditCourse = async (courseId: string, updatedData: any) => {
        startTransition(() => {
            dispatch(updateCourse({ courseId, courseData: updatedData, headers: { Authorization: `Bearer ${authToken}` } }))
                .then(() => dispatch(fetchCoursesByCreator({ creatorId, authToken })));
        });
    };

    const handleTogglePublishStatus = async (courseId: string, currentStatus: boolean) => {
    ...
    return (
        <div className="min-h-screen">
            <div className="flex sm:flex-row flex-col justify-between items-center gap-3 px-[40px]">
                <h1 className="text-[#202224] text-[24px] leading-[40px] font-semibold">My Courses</h1>
            ...
            <FormProvider {...methods}>
                <form onSubmit={methods.handleSubmit((data) => console.log(data))}>
                    <div className={`mt-[30px] px-[40px] max-w-7xl mx-auto gap-6 ${loading ? 'flex justify-center items-center min-h-[55vh]' : 'grid sm:grid-cols-3 items-center justify-center'}`}>
                        {loading ? (
                            <Loader />
                        ) : error ? (
                            ...
                            creatorCourses.map((course: Course) => (
                                <CourseCard
                                    ...
                                    onTogglePublishStatus={() => handleTogglePublishStatus(course?._id, course?.isPublished)}
