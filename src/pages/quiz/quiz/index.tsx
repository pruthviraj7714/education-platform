...
function QuizDetail({}: Props) {
  ...
  const { loading, error, quiz, questions } = useSelector(
  ...
  const handleModalOk = async () => {
    if (editQuestion && courseId && quizId) {
      try {
        const updatedData = await editQuestionApi(
          courseId,
          quizId,
          editQuestion
        );
  ...
  const handleRemoveOption = (index: number) => {
    setEditQuestion((prev: any) => {
      const updatedOptions = prev.solution.options.filter(
    ...
    <div className="mx-[80px] my-[30px] p-6 bg-white rounded-lg shadow-md border">
                    ...
                    <ul className="mt-2 space-y-2">
                      {question.questionType === "TEXT" ? (
                        <Input
                          placeholder="Answer"
                          value={
                            (selectedAnswers[question._id] as string) || ""
                          }
                          onChange={(e) =>
                              ...
                              <input
                                ...
                                checked={
                                  Array.isArray(selectedAnswers[question._id])
                                    ? selectedAnswers[question._id]?.includes(
                                        option
                                      )
      ...
      >
        ...
        {editQuestion?.questionType !== "TEXT" && (
          <>
            {editQuestion?.solution?.options?.map(
              (option: string, index: number) => (
                <div key={index} className="mb-2 flex items-center">
                  ...
                  <Radio
                    value={option}
                    checked={
                      editQuestion.solution.solution === option ||
                      (Array.isArray(editQuestion.solution.solution) &&
                        editQuestion.solution.solution.includes(option))
                    }
