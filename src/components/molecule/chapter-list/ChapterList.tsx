...
import { updateChapter, deleteChapter, getCourseChapter } from '../../../redux/slices/mentorSlice';
...
const ChapterList: React.FC<ChapterListProps> = ({ chapters, setChapters }) => {
    ...
    const handleSaveChapter = async (newChapter: Chapter, event?: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            if (courseId && authToken) {
                const fetchedChapters = await dispatch(getCourseChapter({ courseId, authToken })).unwrap();
        ...
        <div className="bg-white rounded-md w-full">
            <div className='space-y-4 border-[#E4E4E7]'>
                {isAddingChapter || editingChapterIndex !== null ? (
                    <CourseDetails
                        chapter={isAddingChapter ? { title: '', content: '' } : chapters[editingChapterIndex!]}
                    ...
                    <>
                        <div className='flex items-center justify-between'>
                            <h2 className='text-[20px] leading-[28px] font-semibold text-[#191919]'>Chapter List</h2>
                            <div className='flex items-center gap-2'>
                                <Button className='flex items-center gap-2' variant='outlined' onClick={addChapter}>
                                    <IoMdAdd /> Add Chapter
                                </Button>
                            </div>
                        </div>
                        <ul className='space-y-4 pt-4'>
                            {chapters.map((chapter, index) => (
                                <li key={chapter._id} className='cursor-pointer flex justify-between items-center border-b border-[#E4E4E7] pb-4'>
                                    <span onClick={() => handleViewChapter(courseId || '', chapter._id || '')}>{chapter.title}</span>
                                    <div className='flex items-center gap-2'>
